...
def train_mobilenet(
    ...
    for combo in combinations:
        ...
        if hasattr(torch, "compile"):
            mobilenet_model = torch.compile(mobilenet_model)
        #parameter_report(mobilenet_model)
        #summary(mobilenet_model, (1, 28, 28))

        # Compute class weights for imbalanced dataset
        class_weights = _compute_class_weights(train_data, len(classes))
        loss_fn = nn.CrossEntropyLoss(weight=class_weights.to(device))
        optimizer = torch.optim.AdamW(mobilenet_model.parameters(), lr=lr, weight_decay=1e-4)
        ...
        for t in range(EPOCHS+30):  # Allow extra epochs for potential early stopping   
            print(f"Epoch {t+1}\n-------------------------------")
            total_loss, epoch_energy, epoch_mem = mobilenet_model.train_model(train_dataloader, loss_fn, optimizer, device, monitor)
            total_energy.append(epoch_energy)
            mem_utilized.append(epoch_mem)
            acc, avg_loss, avg_flops, _, _, _, _, _ = mobilenet_model.test_model(test_dataloader, loss_fn, device)
        ...
        mobilenet_model.save_model(model_dir / f"mobilenet_model_{lr}.pth")
        ...
        plot_metrics(t,
                    ...
                    accuracy[:t], 
                    total_energy[:t], 
                    model_dir / f"mobilenet_metrics_{lr}.png"
                    )
        ...
        save_metrics(t, 
                    ...
                    mem_utilized[:t],
                    flops[:t],
                    model_dir / f"mobilenet_metrics_{lr}.csv"
                    )
        save_txt(
            f"{round((mem_after - mem_before) / 1024**2, 2)}MB\n{sum(flops)/len(flops):.2f}FLOPS",
            model_dir / f"mobilenet_memory_utilization_{lr}.txt",
        )

def evaluate_mobilenet_model(model_class, model_path, lr, labels, test_data, device):
    """Evaluate the best MobileNet model based on early stopping and return performance metrics."""
    mobilenet_model = model_class(
    ...
    _, _, avg_flops, energy, mem_utilization, params, all_preds, all_labels = mobilenet_model.test_model(test_dataloader, loss_fn, device)
    confusion_matrix(all_preds, all_labels, labels, model_path.parent/f"mobilenet_confusion_matrix_{model_path.stem}.png")
